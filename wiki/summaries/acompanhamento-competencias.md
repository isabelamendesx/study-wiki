...
### Technical Depth
- **Páginas:** 0
- **Próximos passos:** Detalhes da stack principal, gerenciamento de memória, compilação, boas práticas ponta a ponta.

### Persistence
- **Páginas:** 0
- **Próximos passos:** SQL/NoSQL, otimização de queries e índices, replicação, transações.
...
### AI Engineering
- **Páginas:** 0
- **Foco:** Code agents, AI copilots, prompt engineering, MCP, AI safety.

---

## Como esta página é atualizada

A LLM atualiza automaticamente esta página quando:

1. Uma nova página de conceito é criada → incrementa a contagem de páginas
   nas áreas listadas no campo `competencies`.
2. Uma página existente é atualizada com novo conteúdo → revisa `understanding`.
3. A Isabela demonstra compreensão em autoavaliação → a LLM sugere ajustar
   `understanding` de `low` para `medium` ou `high`.
