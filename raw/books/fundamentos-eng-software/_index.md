# Fundamentos de Engenharia de Software — Progresso de Leitura

**Livro:** Fundamentos de Engenharia de Software: Uma abordagem de engenharia
**Autor**: Mark Richards e Neal Ford
**Editora:** O'Reilly
**Início:** 2026-06-18
**Status:** 📖 Em leitura

## Capítulos

<!-- LLM: marcar como lido quando a Isabela terminar cada capítulo e dropar as notas em raw/ -->

- [x] Cap 1 e 2: Introdução e Pensamento Arquitetônico — lido em 2026-06-21, [[arquitetura-de-software|Arquitetura de Software]]
- [x] Cap 3: Modularidade — lido em 2026-06-21, [[../../../wiki/concepts/modularidade|Modularidade]], [[conascencia|Conascência]]
- [x] Cap 4: Definição das Características da Arquitetura — lido em 2026-07-01, [[../../../wiki/concepts/caracteristicas-arquiteturais|Características Arquiteturais]]
- [x] Cap 5: Identificando Características Arquiteturais — lido em 2026-07-06, [[../../../wiki/concepts/identificacao-caracteristicas-arquiteturais|Identificação de Características Arquiteturais]]
- [x] Cap 6: Medindo e Controlando as Características Arquiteturais — lido em 2026-07-08, [[../../../wiki/concepts/medicao-caracteristicas-arquiteturais|Medição de Características Arquiteturais]], [[../../../wiki/concepts/fitness-functions|Fitness Functions]]
- [x] Cap 7: Escopo das Características Arquiteturais — lido em 2026-07-08, [[../../../wiki/concepts/architecture-quantum|Architecture Quantum]]
- [ ] Cap 8:
- [ ] Cap 9:
- [ ] Cap 10:
- [ ] Cap 11:
- [ ] Cap 12:

<!-- A LLM deve adicionar datas conforme os capítulos forem lidos:
     - [x] Cap N: Título — lido em YYYY-MM-DD, [[link-para-pagina-de-conceito|Conceito principal]] -->
